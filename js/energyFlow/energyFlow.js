(function (root, factory) {
            ...
            $.each(config.nodes, function (id, node) {
                if (isInRange(canvas, e, node)) {
                    if (node.click && typeof(node.click) == "function") {
                        //节点连续多次点击优化(多次点击只响应一次)
                        var _oldSelectedNodeId = null;
                        for (var i = 0; i < config.nodes.length; i ++) {
                            if (config.nodes[i].status == "selected") {
                                _oldSelectedNodeId = config.nodes[i].id;
                                break;
                            }
                        }
                        if (_oldSelectedNodeId && _oldSelectedNodeId == node.id) return;
    ...
    function drawBorder(ctx, node, lineDashSupport) {
        ...
        var lineWidth = 3;//擦除边框(白色线条)的像素大于边框像素(若小于,则擦除不干净)
    ...
    function needReCreate(curConfig, preConfig){
        ...
        for(var i = 0 ; i<curConfig.nodes.length; i++){
            if (parseInt(curConfig.nodes[i].id) != parseInt(preConfig.nodes[i].id)
                || parseInt(curConfig.nodes[i].width) != parseInt(preConfig.nodes[i].width)
                || parseInt(curConfig.nodes[i].height) != parseInt(preConfig.nodes[i].height)) {
                return true
            }
        }
    ...
    EnergyFlow.prototype = {
        ...
        init: function () {
            ...
            var animate = function () {
                if($("#"+$(_this.canvas).attr("id")).length == 0 || $('canvas[EFID="'+_this.id+'"]').length == 0 ){
                    _this.destroy();
                    return;
                }
