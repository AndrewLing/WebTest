...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        p = $.extend({ //apply default properties
            ...
            title: false,
            idProperty: 'id',
            pagestat: '{total} items',
            pagetext: 'Page',
            outof: 'of',
            findtext: 'Find',
            params: [], //allow optional parameters to be passed around
            procmsg: 'Processing, please wait ...',
            beforePageText: Msg.gridParam.beforePageText,
            jumpTo: Msg.gridParam.jumpTo,
            mPageText: Msg.gridParam.mPageText,
            pageShowCount: Msg.gridParam.pageShowCount,
            query: '',
            qtype: '',
            nomsg: 'No items',
            ignore: 0,
            lastTh: '',
            textOverflowHide: true,
            inDialog: false,
            ...
            singleSelect: false, //single or multi select
            select: true,//not select grid
        ...
        var g = {
            ...
            dragStart: function (dragtype, e, obj) { //default drag function start
                ...
                } else if (dragtype == 'colMove' && p.colMoveAble) {//column header drag
            ...
            dragEnd: function () {
                if (this.colresize) {
                    ...
                    $('tr:visible', this.bDiv).each(function () {
            ...
            /**
             * zwx222076 调整table的宽度和高度
             */
            adjust: function (data) {
                g.adjustWidth(data);
                g.adjustHeight();
//                g.isShowScrollX("adjust");
                g.rePosDrag();
                g.fixHeight();
            },
            /**
             * zwx222076 为table添加最高宽度限制
             */
            adjustHeight: function () {
                if (p.height == 'auto') {
                    var dw = document.documentElement.clientWidth;
                    var dh = document.documentElement.clientHeight;
                    var newH = parseInt($(t).css("height"));

                    var node = t;
                    var absH = node.offsetTop;
                    while (node = node.offsetParent) {
                        absH += node.offsetTop;
                    }
                    p.maxheight = dh - absH - 60;
                    if (newH > p.maxheight && p.maxheight > 0) {
                        g.bDiv.style.height = p.maxheight + 'px';
                    } else {
            ...
            /**
             * zwx222076 调整table的宽度
             */
            adjustWidth: function (data) {
                if (p.overflowx == 'auto')
                    return;
                if (p.width == 'auto') {
                    if (p.inDialog) {
                        p.width = parseInt($(g.bDiv).css("width")) - 1;
                        g.bDiv.style.width = p.width + 'px';
                        g.hDiv.style.width = p.width + 'px';
                    }
                }
//                var bw = $(g.hDiv).width() || 0;
//                if (bw <= 0) return;
//                var btw = $('table', g.hDiv).width() || 100;
//                var ths = $("th", g.hDiv);
//                for (var i = 0; i < ths.length - 1; i++) {
//                    var ow = $('div', ths[i]).eq(0).width();
//                    var nw = ow * bw / btw + 0.5;
//                    $('div', ths[i]).eq(0).width(nw);
//                }

                var ths = $(g.hDiv).find("th:visible");
                var ow = ths.parent().width();
                var nw = $(g.hDiv).width();
                var i = ths.length - 1;
                var dif;
//                if (parseInt($('table', g.bDiv).eq(0).css("width")) == 0)
//                	thdif = 0;
//                else
                var thdif = nw - ow;
                var div = Math.round(thdif / ths.length);
                var thwidth;
                for (var int = 0; int < ths.length - 1; int++) {
                    var thw = $(ths[int]).css("width");
                    if($(ths[int]).find('input:checkbox') || $(ths[int]).find('input:radio')) {
                        continue;
                    }
                    if (parseFloat(thw) < 1) {
                        $(ths[int]).css("width", parseFloat(thw) * 100 + '%');
                    }
                    thdif -= div;
                    thwidth = parseInt($(ths[int]).css("width")) + div;
                    $(ths[int]).attr("width", thwidth);
                    $('div', ths[int]).css("width", thwidth - 9 + "px");
                }
                thwidth = parseInt($(ths[i]).css("width")) + thdif;
                $(ths[i]).attr("width", thwidth);
                $('div', ths[i]).css("width", thwidth - 8 + "px");

                //g.addData(data);
            },
            setbDivOverflowx: function (data) { //parse data
                $(g.bDiv).css('overflow-x', data);
            },
            addData: function (data) { //parse data
                ...
                if (p.dataType == 'json') {
                    $.each(data.rows, function (i, row) {
                        ...
                        if (p.checkbox) {
                            ...
                            var cthch;
                            if (p.singleSelect) {
                                cthch = $('<input type="radio" name="single" value="' + $(tr).attr('id') + '"/>');
                            } else {
                                cthch = $('<input type="checkbox" value="' + $(tr).attr('id') + '"/>');
                            }
                        ...
                        $('thead tr:first th', g.hDiv).each(function () {
                            ...
                            if (typeof row.cell == 'undefined') {
                                td.innerHTML = g.escapseHtml(row[p.colModel[idx].name]);
                                if (p.colModel[idx].cssclass) {
                                    $(td).attr("value", td.innerHTML);
                                    $(td).addClass(p.colModel[idx].cssclass);
                                    var theValue = row[p.colModel[idx].name];
                                    var taga = ' <a href="#" >' + theValue + '</a>';
                                    td.innerHTML = taga;
                                }
                            } else {
                        ...
                        if ($('thead', this.gDiv).length < 1 && row.cell) {//handle if grid has no headers
                ...
                $('tr', t).unbind();
                if (p.checkbox) {
                    $(g.hDiv).find('input')[0].checked = false;
                }
                $(t).empty();
                $(t).append(tbody);
                this.addCellProp();
                ...
                /*	if($(t).height()>$(this.gDiv).parent().height()){
                 $(this.bDiv).css("height",$(this.gDiv).parent().height());
                 }else{
                 $(this.bDiv).css("height","auto");
                 }*/
                if (p.onLoadReady) {
                    var trs = $(t).find('tr');
                    p.onLoadReady(datas, trs);
                }
                //zhangshuo 屏蔽拖动功能
                if(!p.draggable){
                	$('.cDrag').hide();
            ...
            buildpager: function () { //rebuild pager based on new properties
                ...
                $('.pcontrol label', this.pDiv).html(p.page);

                var showTotal = p.outof;
                showTotal = showTotal.replace(/\{totalPage\}/g, p.pages);
                $('.pcontrol span', this.pDiv).html(showTotal);
...
//                stat = stat.replace(/{to}/, r2);
            ...
            populate: function () { //get latest data
                ...
                var param = {"page": p.newp, "rp": p.rp, "sortname": p.sortname, "sortorder": p.sortorder, "query": p.query, "qtype": p.type };
                if (p.params) {
                    param = $.extend({}, p.params, param);
                }
                $.omcAjax(p.url, param, function (data) {
                    var ro = data;
                    if (ro.success) {
                        ro = ro.data;
                        var nd = {
                    ...
                    //如果只有一页或者最后一页或者最前面一页时候，需要将按钮置为不可用
                    if ((ro.pageNo == 1) && (ro.pageCount == 1 || ro.pageCount == 0) || (ro.pageNo == undefined && ro.pageCount == undefined)) { //只有一页，分页控件全部不可用
                        $('.pFirst', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pPrev', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pNext', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pLast', g.pDiv).css('background-color', '#e0e0e0');
                        //$('.pcontrol input', g.pDiv).attr('disabled', true);
                    } else if (ro.pageNo == ro.pageCount) { //向后一页，最后一页不可用
                        $('.pFirst', g.pDiv).css('background-color', '');
                        $('.pPrev', g.pDiv).css('background-color', '');
                        $('.pNext', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pLast', g.pDiv).css('background-color', '#e0e0e0');
                        //$('.pcontrol input', g.pDiv).attr('disabled', false);
                    } else if (ro.pageNo == 1 && ro.pageCount > 1) {//多页，现在停留在第一页，则第一页不可用
                        $('.pFirst', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pPrev', g.pDiv).css('background-color', '#e0e0e0');
                        $('.pNext', g.pDiv).css('background-color', '');
                        $('.pLast', g.pDiv).css('background-color', '');
                        //$('.pcontrol input', g.pDiv).attr('disabled', false);
                    } else {
                        $('.pFirst', g.pDiv).css('background-color', '');
                        $('.pPrev', g.pDiv).css('background-color', '');
            ...
            doSearch: function () {
                ...
                $('select[type=text]', g.pDiv).val('1');
                p.newp = 1;
                this.populate();
            },
            /**
             * zwx222076 根据table的宽度来决定要不要显示滚动条
             */
            initScrollX: function () {
                ...
                var sum = 0;
                var i = 0;
                var totleW = parseInt($(t).parent().css('width'));
                for (i = 0; i < ths.length; i++) {
                    sum = parseInt(sum) + parseInt($(ths[i]).css('width')) + 2;
                }
                if (p.checkbox)
                    sum = parseInt(sum) + 22;
            ...
            isShowScrollX: function (po) {
                var gridW = parseInt($(t).parent().css('width')) + 2;
                var gridH = parseInt($(t).parent().css('height')) + 2;
                var tH = parseInt($(t).css('height'));
                var tW = parseInt($(t).css('width'));
                if (tW > gridW) {
                    p.overflowx = "auto";
                } else {
                    p.overflowx = "hidden";
                ...
                if (po != p.overflowx) {
                    if (p.overflowx == "auto" && tH <= gridH && tH + 20 > gridH && p.height == 'auto') {
                        var h = parseInt($(g.bDiv).css('height'));
                        $(g.bDiv).css('height', h + 20);
                    } else if (p.overflowx == "hidden" && tH <= gridH && tH + 20 <= gridH && p.height == 'auto') {
                        var h = parseInt($(g.bDiv).css('height'));
                        $(g.bDiv).css('height', h - 20);
                    }
            ...
            /**
             * zwx222076 将百分比宽度改成真实宽度
             */
            changeWidth: function (th, perW) {
                var totleW = parseInt($(t).parent().css("width"));
                if (perW == 0) {
                    $(th).attr('width', parseInt($(th).attr('width')) + Math.round(p.ignore));
                    p.ignore = 0;
                } else {
                    var totle = totleW;
                    if (p.width != 'auto')
                        totle = p.width;
                    var interval = 13;//单元格间距
                    var checkBox = p.checkbox ? 54 : 29;
                    var width;
                    if (perW > 1) {
                        width = perW / 100 * (totle - checkBox) - interval;
                        $(th).attr('width', width);
                    } else {
                        width = perW * (totle - checkBox) - interval;
                        $(th).attr('width', width);
                    }
                    p.ignore += width - parseInt(width);
                }
            },
            isInDialog: function () {
                var node = t;
                while (node = node.offsetParent) {
                    if ($(node).hasClass("ui-dialog") == true) {
                        p.inDialog = true;
                        break;
                    }
                }
            ...
            changePage: function (ctype) { //change page
                ...
                switch (ctype) {
                    ...
                    case 'input':
                        ...
                        $('.pcontrol label', this.pDiv).html(nv);
                        p.newp = nv;
                        break;
                }
                if (p.newp == p.page) {
            ...
            addCellProp: function () {
                $('tbody tr td', g.bDiv).each(function () {
                    ...
                    if (p.textOverflowHide) {
                        $(tdDiv).css({'text-overflow': 'ellipsis', 'overflow': 'hidden'});
                    }
                    ...
                    if (pth != null) {
                        var record = g.findRecordById(p, pid);
                        if (pth.process) pth.process(tdDiv, pid, record);
                    }
                    //zhangshuo 2015.3.26
                    if($(this).hasClass('cth')){
                    	var width=parseInt($(tdDiv).css('width'))+5+'px';
                    	$(tdDiv).css({'padding':'0px','margin-left':'5px','width':width});
                    }
                    $(this).empty().append(tdDiv).removeAttr('width'); //wrap content
                    g.addTitleToCell(tdDiv);
                });
            },
            findRecordById: function (p, id) {
                var record;
                if (p.data && p.data.rows) {
                    $(p.data.rows).each(function (t, e) {
                        if (e[p.idProperty] == id) {
                            record = e;
                        }
                    });
                }
...
//                    if (obj.href || obj.type) return true;
                    ...
                    if (p.singleSelect && !g.multisel) {
                        ...
                        $.each($(this).siblings(), function (t, e) {
                            $(this).find('input')[0].checked = false;
                        });
                    }
                    if (p.onSingleClick) {
                        p.onSingleClick(this);
                    }
                ...
                if (cm) {	// only use cm if its defined
                    ...
                    if (cm.width) {
                        if ((cm.width + "").indexOf("%") > -1) {
                            g.changeWidth(th, parseInt(cm.width));
                            lastth = th;
                        } else if (cm.width < 1) {
                            g.changeWidth(th, cm.width);
                            lastth = th;
                        }
                        else {
        ...
        $('thead tr:first th', g.hDiv).each(function () {
            ...
            if (p.textOverflowHide) {
                $(thdiv).css({'text-overflow': 'ellipsis', 'overflow': 'hidden'});
            }
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<div class="pGroup"><span class="pPageStat"></span></div>   <div style="float: right;"> <div class="pGroup"> <div class="pFirst pButton"><span></span> </div> <div class="pPrev pButton"><span></span></div><div class="pGroup"><span class="pcontrol"> <label type="text" size="4">1</label></span> </div> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="pGroup"><span class="pcontrol">' + p.beforePageText + ' <label type="text" size="4">1</label> <span> ' + p.outof.replace(/{totalPage}/g, '1') + ' </span>&nbsp;&nbsp;&nbsp;' + p.jumpTo + '<input type="text" size="4" value="1">' + p.mPageText + ' </span></div></div>';
            ...
            if (p.useRp) {
                ...
                $('.pDiv2', g.pDiv).prepend("<div class='pGroup'><span>" + p.pageShowCount + "</span><select name='rp'>" + opt + "</select></div>");
            ...
            if (p.checkbox) {
                $('tr', g.hDiv).each(function () {
                    ...
                    if (!p.singleSelect) {
                        ...
                        cthch = $('<input type="radio" disabled="true"/>');
                    }
                    if (cthch) {
    ...
    $.fn.flexReload = function (p) { // function to reload grid
        return this.each(function () {
            if (p) {
                if (p.newp) this.p.newp = p.newp;// 添加刷新页码初始化，供当前查询调用
    ...
    $.fn.flexAdjust = function () {
        this.each(function () {
            if (!this.grid) {
                return false;
            }
            this.grid.adjust(this.p.data);
        });
    }; //end flexAdjust
    ...
    $.fn.selectedRecords = function () {
        ...
        this.each(function () {
            if (this.grid) {
                ...
                currentId = this.p.idProperty;
            }
        });
        $(selected).each(function (d, p) {
            var indexID = p[0].RowIdentifier;
            $.each(griddata.rows, function (i, row) {
                if (row[currentId] == indexID) {
                    records.push(row);
                }
            });
        });
    ...
    $.fn.selectedRow = function (row) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
        var arReturn = [];
        var arRow = [];
        var selector = row;
        //console.log(this.selector);
     ...
     * 获取选中记录的JSON原型格式
     */
    $.fn.selectedRecord = function (row) {
        var records = [];
        var selected = this.selectedRow(row);
     ...
     * 通过指定行的id值及和列名获取该列的值
     */
    $.fn.getCellByCoordinate = function (rowid, colName) { // function to get row's data from grid
        var griddata = {
            total: 1,
            page: 1,
            rows: []
