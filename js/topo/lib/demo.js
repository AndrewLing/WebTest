...
function fixNodeLocationDemo (node) {
...
demo.Util = {
    ...
    addDraggableButton: function (div, name, src, className,nodeTypeName,nodeTypeId) { //閿熺潾闈╂嫹閿熷壙纰夋嫹
        var srcName = src;
    	var image = new Image();
        ...
        image.style.padding = '3px 3px 3px 3px';
        image.style.width = "40px";
        image.style.height = "40px";
        if (src.indexOf('/') < 0) {
            src = '/module/topo/images/toponodeicon/' + src + '.png';
        }
        image.setAttribute('src', src);
        image.addEventListener('dragstart', function (e) {
            e.dataTransfer.effectAllowed = 'copy';
            e.dataTransfer.setData('Text', 'className:' + className);
            e.dataTransfer.setData('srcName', srcName);
            e.dataTransfer.setData('nodeTypeName', nodeTypeName);
            e.dataTransfer.setData('nodeTypeId', nodeTypeId);
        }, false);
    ...
    align: function (elements, alignType) {
        ...
        elements.forEach(function(node){
        	if(node instanceof topo.Text){
            	var textSize = _twaver.g.getTextSize(node.getStyle('label.font'), node.getName());
            	var nodeBounds = node.getRect();
                var position = _twaver.position.get(node.getStyle('label.position'), nodeBounds, textSize);
                var labelxoffset = node.getStyle("label.xoffset");
                var labelyoffset = node.getStyle("label.yoffset");
                var rectoffset = {x: position.x + labelxoffset, y: position.y + labelyoffset, width: textSize.width, height: textSize.height};
            	if(alignType == "left"){
            		if(!("right.right" == node.getStyle('label.position'))){ 
            			node.setStyle("label.position","right.right");
            			node.setLocation(rectoffset.x,node.getY());
            		}
            		
                }
            	else if(alignType == "right"){
            		if(!("left.left" == node.getStyle('label.position'))){  
            			node.setStyle("label.position","left.left");
            			node.setLocation(rectoffset.x + rectoffset.width,node.getY());
            		}
            		
            	}
            	else if(alignType == "top"){
            		if(!("bottom.bottom" == node.getStyle('label.position'))){  
            			node.setStyle("label.position","bottom.bottom");
            			node.setLocation(node.getX(),rectoffset.y);
            		}
            	}
            	else if(alignType == "bottom"){
            		if(!("top.top" == node.getStyle('label.position'))){
            			node.setStyle("label.position","top.top");
            			node.setLocation(node.getX(),rectoffset.y + rectoffset.height);
            		}
            	}
            }
        });
        ...
        elements.forEach(function (node, index, array) {
            ...
            if(node instanceof twaver.Follower || node instanceof twaver.Grid){
            	return ;
            }
            var x = node.getX();
            var y = node.getY();
    ...
    createDraggableNetwork: function (box,topoconfigobj) {
        ...
        network.getView().addEventListener('drop', function (e) {
            ...
            var srcName = e.dataTransfer.getData('srcName');
            var nodeTypeId = e.dataTransfer.getData('nodetypeid');
            var nodeTypeName = e.dataTransfer.getData('nodetypename');
            if (!text) {
                return false;
            }
            if (text && text.indexOf('className:') == 0) {
            	
                demo.Util._createElement(network, text.substr(10, text.length), network.getLogicalPoint(e),srcName,nodeTypeName,nodeTypeId,topoconfigobj);
            }
    ...
    _createElement: function (network, className, centerLocation,srcName,nodeTypeName,nodeTypeId,topoconfigobj) {   
        var element = twaver.Util.newInstance(className);
        element.setCenterLocation(centerLocation);
        element.setClient("nodetypename",nodeTypeName);
        element.setClient("nodetypeid",nodeTypeId);
        if(className == "twaver.Node"){
        	element.setImage(srcName);
        }
        element.setParent(network.getCurrentSubNetwork());
        if(className == "demo.Shelf23"){                             
	        var children = element.getChildren();   
	        children.forEach(function(e){
	        	network.getElementBox().add(e);
	        });
        }
        if(className == "topo.Shelf22"){                            
	        var children = element.getChildren();   
	        children.forEach(function(e){
	        	network.getElementBox().add(e);
	        });
        ...
        //缂佸嫭锟介梽鎰煑
        var topographid = topoconfigobj.newConfSchemaObj.schemeTypeId;
        
        //濡剝婢橀崶鎯у帗閹锋牕鍙嗛梽鎰煑
        var treenodetype = topoconfigobj.treenodetype;
        
        if(className == "topo.JZZuChuan"){  
        	if(treenodetype == "node" && topographid != 6 && topographid){
        		/*var children = element.getChildren();   
        		children.forEach(function(e){
        			network.getElementBox().add(e);
        		});*/
        		for(var i=0;i<8;i++){
        	    	for(var j=0;j<2;j++){
        	    		...
        	    		cell.setHost(element);
        	    		element.addChild(cell);
        	    		network.getElementBox().add(cell);
        	    	}
        	    }
        	}
        }
        
        if(treenodetype == "tpl"){                                    //濡剝婢橀懞鍌滃仯
        	 //$("#alertdlg").dialog({height:180});
        	 $("#alertmsg").html("");
			 $("#alertmsg").html(Msg.topocfg.templatenoedit);     //鐠囩兘锟介幏鈺呮姜閻㈢數鐝懞鍌滃仯缂佹ê鍩楃紒鍕拷閸ワ拷
             $("#alertdlg").dialog("open");
             return false;
        }
        
        
        if((topographid == undefined) || (topographid == null) ){
        	 $("#alertdlg").dialog({height:200});
        	 $("#alertmsg").html("");
			 $("#alertmsg").html(Msg.topocfg.chosenotstationnode);     //鐠囩兘锟介幏鈺呮姜閻㈢數鐝懞鍌滃仯缂佹ê鍩楃紒鍕拷閸ワ拷
             $("#alertdlg").dialog("open");
             return false;
        }
        if((topographid == 6)|| (topographid == "")){
        	 $("#alertdlg").dialog({height:200});
       	     $("#alertmsg").html("");
			 $("#alertmsg").html(Msg.topocfg.chosenotstationnode);      //鐠囩兘锟介幏鈺呮姜閻㈢數鐝懞鍌滃仯缂佹ê鍩楃紒鍕拷閸ワ拷
             $("#alertdlg").dialog("open");
      ...
      /*  if((topographid == 2) && (nodeTypeId == 19)){ //閸掑棗灏幏鏍уЗtable
        	alert(element.getChildren().size());
        	return false;
        }
        */
        
        if(topographid == 2){           //2閸掑棗灏�
        	/*if(!((nodeTypeId == 9) || (nodeTypeId == 13) || (nodeTypeId == 4) || (nodeTypeId == 7))){
        		 $("#alertmsg").html("");
    			 $("#alertmsg").html("閸掑棗灏猾璇茬�风紒鍕拷閸ュ彞绗夐懗鑺ュ潑閸旂姵顒濈猾璇茬�烽崶鎯у帗!");
                 $("#alertdlg").dialog("open");
                 return false;
        	}*/
        }
        if(topographid == 3){           //3鐎涙劙妯�
        	/*if(!((nodeTypeId == 6) || (nodeTypeId == 8) || ((nodeTypeId == 5)&&(className == "topo.NiBianQi")) || (nodeTypeId == 7)|| (nodeTypeId == 15) || (nodeTypeId == 16) || (nodeTypeId == 18))){
        		 $("#alertmsg").html("");
    			 $("#alertmsg").html("鐎涙劙妯�缁鐎风紒鍕拷閸ュ彞绗夐懗鑺ュ潑閸旂姵顒濈猾璇茬�烽崶鎯у帗!");
                 $("#alertdlg").dialog("open");
                 return false;
        	}*/
        	/*var curbox = network.getElementBox();
        	var idFinder = new twaver.QuickFinder(curbox,"nodetypeid" ,"client");
			var nodes = idFinder.find("18");  
			if(nodes && nodes.size() > 0 && (nodeTypeId == 18)){
				 $("#alertmsg").html("");
    			 $("#alertmsg").html("娑撳秷鍏樺ǎ璇插婢舵矮閲滈弫浼村櫚閸ユ儳鍘�!");
                 $("#alertdlg").dialog("open");
                 return false;
			}*/
        }
        if(topographid == 5){           //5闁棗褰夐崳銊ュ瀻閸ワ拷
        	/*if(!((nodeTypeId == 20) || (nodeTypeId == 19) || (nodeTypeId == 14)|| ((nodeTypeId == 5)&&(className == "topo.NiBianQi3D")) || (nodeTypeId == 7)|| (nodeTypeId == 25) )){
        		 $("#alertmsg").html("");
    			 $("#alertmsg").html("闁棗褰夐崳銊ュ瀻閸ュ墽琚崹瀣矋閹礁娴樻稉宥堝厴濞ｈ濮炲銈囪閸ㄥ娴橀崗锟�");
                 $("#alertdlg").dialog("open");
                 return false;
        	}*/
        }
        
       /* if(className == "topo.Table"){                              
        	var children = element.getChildren();   
 	        children.forEach(function(e){
 	        	network.getElementBox().add(e);
 	        });
        	
        	element.addPropertyChangeListener(function (e) {
        		
        		if(e.property == "S:grid.row.count" || e.property == "S:grid.column.count"){
        			var rowcount = element.getStyle("grid.row.count");
            		var colcount = element.getStyle("grid.column.count");
            		 var followers = element.getFollowers();
                     if(followers != null){
                         var len = followers.size();                                                    
                         for(var i = len - 1;i >= 0 ;i--){
                        	 network.getElementBox().remove(followers.get(i));
                         }
                     }
                     
                     for(var i = 0; i < colcount;i++){
            	    	 for(var j = 0; j < rowcount;j++){
            	    		 var cell=new topo.Text();
            	    		 cell.setStyle("follower.column.index",i);
            	    		 cell.setStyle("follower.row.index",j);
            	    		 cell.setClient("nodetypeid","7");
            	    		 cell.setClient("nodetypename","闂堟瑦锟介弫鐗堝祦");
            	    		 cell.setHost(element);
            	    		 network.getElementBox().add(cell);
            	 	    }
            	    }
        ...
        if((topographid == 5) &&(nodeTypeId == 5) && (className == "topo.NiBianQi3D")){                 //闁棗褰夐崳銊ュ瀻閸ワ拷
        	var curbox = network.getElementBox();
        	var idFinder = new twaver.QuickFinder(curbox,"nodetypeid" ,"client");
			var nodes = idFinder.find("5");  
			if(nodes && (nodes.size() > 0)){
				 nodes.forEach(function(e){
					 var imagetemp = e.getImage();
					 if(imagetemp == "nibianqi3d"){
						 returnflag = true;
					 }
				 })
			}
        ...
        if(returnflag){
        	$("#alertdlg").dialog({height:180});
        	$("#alertmsg").html("");
		    $("#alertmsg").html(Msg.topocfg.cannotaddnibianqinode);                   //娑撳秷鍏樺ǎ璇插婢舵矮閲滈柅鍡楀綁閸ｃ劌娴橀崗锟�
            $("#alertdlg").dialog("open");
            return false;
        }
       
        if((topographid == 4)&&(nodeTypeId == 13) && ((className == "topo.SanXiang3D")||(className == "topo.LiangXiang3DLJ")||(className == "topo.LiangXiang3DSJ"))){   //缁犲崬褰夐崚鍡楁禈
        	var curbox = network.getElementBox();
        	var idFinder = new twaver.QuickFinder(curbox,"nodetypeid" ,"client");
			var nodes = idFinder.find("13");  
			if(nodes && (nodes.size() > 0)){
				 nodes.forEach(function(e){
					 var imagetemp = e.getImage();
					 if((imagetemp == "sanxiang3dr") || (imagetemp == "liangxiang3dsjr") || ((imagetemp == "liangxiang3dljr"))){
						 returnflag = true;
					 }
				 })
			}
        ...
        if(returnflag){
        	$("#alertdlg").dialog({height:180});
        	$("#alertmsg").html("");
		    $("#alertmsg").html(Msg.topocfg.cannotaddsanxiangnode);                   //娑撳秷鍏樺ǎ璇插婢舵矮閲滄稉澶屾祲閸ユ儳鍘�
            $("#alertdlg").dialog("open");
            return false;
        }*/
        
        //娑撹濮╅幒銏＄ゴ閸忓疇浠堥悧鍫熸拱閿涘矂锟介崣妯烘珤閸掑棗娴橀敍宀�顔堥崣妯哄瀻閸ワ拷
       /* if( ((topographid == 5) &&(nodeTypeId == 5) && (className == "topo.NiBianQi3D"))
        	|| ((topographid == 4)&&(nodeTypeId == 13) && ((className == "topo.SanXiang3D")||(className == "topo.LiangXiang3DLJ")||(className == "topo.LiangXiang3DSJ")))){           
        	var objInfo = {pelTypeID: nodeTypeId};
			$.omcAjax("/cm/dev/types", objInfo, function (data) {
                if (data.success == false){
                	     $("#alertmsg").html("");
                	     $("#alertmsg").html(Msg.topocfg.getverlistfail);           //閼惧嘲褰囬悧鍫熸拱閸掓銆冩径杈Е
			             $("#alertdlg").dialog("open");
	     				 return false;
	     			 }
	     			 if(data.success == true){
	     		  		 ...
	     		  		 if(size == 0){
	     		  			 $("#alertmsg").html("");
	     		  			 $("#alertmsg").html(Msg.topocfg.noavaildevver);        //濞屸剝婀侀崣顖滄暏鐠佹儳顦搁悧鍫熸拱
	     		             $("#alertdlg").dialog("open");
	     		  			 return false;
	     		  		 }
	     		  		 else{
                             var dataOptions = data.data;
	     		  			 for(var i = 0;i < size ;i++){
	     		  				 ...
	     		  				 if(idOption == topoconfigobj.newConfSchemaObj.busiObjectId){   //閻楀牊婀癐D閸栧綊鍘�
	     		  					  element.setClient("modelVersionID",idOption);             //閻楀牊婀癐D
	     		  					  element.setClient("modelVersionID",nameOption);           //閻楀牊婀伴崥宥囆� 
	     		  					  break;
	     		  				 }
	     		  			 }
	     		  		 }	
...
demo.Rack = function (id) {               //閿熺潾闈╂嫹閿熷壙纰夋嫹
    ...
    this.setStyle('vector.fill.color', '#C0C0C0');   //#C0C0C0
...
demo.Shelf23 = function (id) {    //閿熺潾闈╂嫹閿熷壙纰夋嫹 2*3閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷锋锟�
   /* demo.Shelf.superClass.constructor.call(this, id);
    ...
    this.setStyle('grid.border', 2);
    this.setStyle('grid.deep', 1);
    this.setWidth(180);
    this.setHeight(80);*/
	demo.Shelf23.superClass.constructor.call(this, id);
    this.setIcon('shelf_icon');
    this.setStyle('grid.row.count', 2);
    this.setStyle('grid.column.count', 3);
    ...
    for(var i = 0;i < 3 ;i++){
    	for(var j = 0;j < 2 ; j++){
    		var cell=new demo.Rack();
    	    cell.setStyle("follower.column.index",i);
    	    cell.setStyle("follower.row.index",j);
    	    /*cell.setStyle("follower.padding",1);
    	    cell.setStyle("grid.deep",1);*/
    	    cell.setHost(this);
    	    cell.setClient("nodeTypeName","閿熶粙涓�");
            cell.setClient("nodeTypeValue","zuchuan");
    	    this.addChild(cell);
    	}
    }
};

twaver.Util.ext('demo.Shelf23', twaver.Grid);


demo.Shelf22 = function (id) {    //閿熺潾闈╂嫹閿熷壙纰夋嫹 2*3閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷锋锟�
	   /* demo.Shelf.superClass.constructor.call(this, id);
	    this.setIcon('shelf_icon');
	    this.setStyle('grid.row.count', 2);
	    this.setStyle('grid.column.count', 3);
	    ...
	    this.setWidth(180);
	    this.setHeight(80);*/
		demo.Shelf22.superClass.constructor.call(this, id);
	    this.setIcon('shelf_icon');
	    this.setStyle('grid.row.count', 2);
...
twaver.Util.ext('demo.Shelf22', twaver.Grid);

demo.TextNode = function (id) {    //閿熸枻鎷烽敓鎺ワ綇鎷烽敓鏂ゆ嫹閿熻鑺傜鎷�
	demo.TextNode.superClass.constructor.call(this, id);
	this.setSize(150,40);
	...
	this.setName("閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷�");
	//this.setCenterLocation(point);
	};

twaver.Util.ext('demo.TextNode', twaver.Grid);



demo.BusNode = function (id) {    //閿熸枻鎷烽敓鎺ワ綇鎷烽敓鏂ゆ嫹閿熻鑺傜鎷�
	demo.TextNode.superClass.constructor.call(this, id);
	this.setSize(500,8);
    this.setStyle('body.type', 'vector');
    this.setStyle('vector.shape', "rectangle");
...
twaver.Util.ext('demo.BusNode', twaver.Node);
