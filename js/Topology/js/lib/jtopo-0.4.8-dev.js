...
!function (window) {
    ...
    var JTopo = {
        ...
        replaceStageWithJson: function (jsonObj) {
            ...
            return scenes.forEach(function (a) {
                ...
                d.forEach(function (a) {
                    ...
                    if ("node" == d) {
                        c = new JTopo.Node;
                        c.alpha = JTopo.config.alpha;
                        c.strokeColor = JTopo.config.nodeStrokeColor;
                        c.fillColor = JTopo.config.fillColor;
                        c.shadow = JTopo.config.shadow;
                        c.shadowColor = JTopo.config.shadowColor;
                        c.font = JTopo.config.font;
                        c.fontColor = JTopo.config.nodeFontColor;
                        c.borderRadius = null;
                        c.shadowOffsetX = JTopo.config.shadowOffsetX;
                        c.shadowOffsetY = JTopo.config.shadowOffsetY;
                ...
                d.forEach(function (a) {
                    ...
                    if ("link" == d) {
                        var nodeA, nodeZ;
                        //找出连接点
                        if (a["fromNode"] && a["toNode"]) {
                            nodes.forEach(function (nodeEle) {
                                    if (nodeEle.elementType == "node") {
                                        if (nodeEle.deviceId == a["fromNode"])
                                            nodeA = nodeEle;
                                        if (nodeEle.deviceId == a["toNode"])
                                            nodeZ = nodeEle;
                                    }
                                }
                            );
                        ...
                        if (nodeA && nodeZ) {
                            ...
                            else if (a["lineType"] == "foldLine") {
                                c = new JTopo.FoldLink(nodeA, nodeZ);
                                c.bundleOffset = JTopo.config.offsetGap;
                            }
                            else if (a["lineType"] == "flexLine") {
                                c = new JTopo.FlexionalLink(nodeA, nodeZ);
                                c.offsetGap = JTopo.config.offsetGap;
                            }
                            else if (a["lineType"] == "curveLine")
                                c = new JTopo.CurveLink(nodeA, nodeZ);

                            c.arrowsRadius = JTopo.config.arrowsRadius;
                            c.lineWidth = JTopo.config.lineWidth;
                            c.alpha = JTopo.config.alpha;
                            c.strokeColor = JTopo.config.lineStrokeColor;
                            c.fillColor = JTopo.config.fillColor;
                            c.shadow = JTopo.config.shadow;
                            c.shadowColor = JTopo.config.shadowColor;
                            c.font = JTopo.config.font;
                            c.fontColor = JTopo.config.lineFontColor;
                            c.lineJoin = JTopo.config.lineJoin;
                            c.shadowOffsetX = JTopo.config.shadowOffsetX;
                            c.shadowOffsetY = JTopo.config.shadowOffsetY;
                        }
                    } else if ("container" == d) {
                        if (a["childNodes"]) {
                            ...
                            if (temp && temp.length > 0) {
                                ...
                                c.alpha = JTopo.config.containerAlpha;
                                c.strokeColor = JTopo.config.nodeStrokeColor;
                                c.fillColor = JTopo.config.containerFillColor;
                                c.shadow = JTopo.config.shadow;
                                c.shadowColor = JTopo.config.shadowColor;
                                c.font = JTopo.config.font;
                                c.fontColor = JTopo.config.fontColor;
                                c.borderColor = JTopo.config.borderColor;
                                c.borderRadius = JTopo.config.borderRadius;
                                c.shadowOffsetX = JTopo.config.shadowOffsetX;
                                c.shadowOffsetY = JTopo.config.shadowOffsetY;
        ...
        createStageFromJson: function (jsonObj, canvas) {
            ...
            JTopo.modeIdIndex = jsonObj.deviceNum;
...
!function (JTopo) {
    ...
    function toJson(a) {
        ...
        d += "frames:" + a.frames + ", scenes:[";

        for (var e = 0; e < a.childs.length; e++) {
            var f = a.childs[e];
            d += "{" + getProperties(f, b) + ", elements:[";
    ...
    function intersection(a, b) {
        var c, d;
        return a.k == b.k ? null :
            (1 / 0 == a.k || a.k == -1 / 0 ? (c = a.x1, d = b(a.x1))
                : 1 / 0 == b.k || b.k == -1 / 0 ? (c = b.x1, d = a(b.x1))
    ...
    function intersectionLineBound(a, b) {
        ...
        null == d && (
            c = JTopo.util.lineF(b.left, b.top, b.right, b.top), d = JTopo.util.intersection(a, c),
            null == d && (
                c = JTopo.util.lineF(b.right, b.top, b.right, b.bottom), d = JTopo.util.intersection(a, c),
                null == d && (
                    c = JTopo.util.lineF(b.left, b.bottom, b.right, b.bottom), d = JTopo.util.intersection(a, c)
                )
            )
        );
...
!function (JTopo) {
    ...
    function Stage(c) {
        ...
        this.toJson = function () {
            var b = this, c = '{"version":"' + JTopo.version + '","deviceNum":"' + JTopo.modeIdIndex + '",';
            if (this.serializedProperties.length) {
...
!function (JTopo) {
    function Scene(c) {
        ...
        this.initialize = function () {
            ...
            this.paintAll = !1;
            var c = "background,backgroundColor,mode,paintAll,areaSelect,translate,translateX,translateY,lastTranslatedX,lastTranslatedY,alpha,visible,scaleX,scaleY,totalLevel".split(",");
...
/************** JTopo.DisplayElement,JTopo.InteractiveElement,JTopo.EditableElement ***************/
!function (JTopo) {
    function b() {
        this.initialize = function () {
            ...
            this.initialize(),
            this.paint = function (a) {debugger
                a.beginPath(),
        ...
        this.mousedragHandler = function (a) {
            ...
            if (null == this.selectedPoint) {
                var b = this.selectedLocation.x + a.dx,
                    c = this.selectedLocation.y + a.dy;
                //画标尺线
                JTopo.utils.createRuleLines(b + this.width / 2, c + this.height / 2);
                //应用自动布局
...
/************** JTopo.Node,JTopo.TextNode,JTopo.LinkNode,JTopo.CircleNode,JTopo.AnimateNode ***************/
!function (JTopo) {
    function b(c) {
        this.initialize = function (c) {
            b.prototype.initialize.apply(this, arguments),
...
/************** JTopo.Link,JTopo.FoldLink,JTopo.FlexionalLink,JTopo.CurveLink ***************/
!function (JTopo) {
    function b(a, b) {
        var c = [];
        if (null == a || null == b)
        ...
        this.initialize = function (b, c, d) {
            if (f.prototype.initialize.apply(this, arguments),
                ...
                null != this.nodeZ && this.nodeZ.inLinks.push(this),
                    ...
                    this.path = [];
                var e = "fromNode,toNode,lineType,text".split(",");
...
!function (JTopo) {
    ...
    b.prototype = new JTopo.InteractiveElement;
    JTopo.Container = b;
}(JTopo);
...
!function (JTopo) {
    ...
    function getRootNodes(b) {
        var c = [],
            d = b.filter(function (b) {
                    return b instanceof JTopo.Link ? !0 : (c.push(b),
                        !1)
                }
            );
        ...
        if (currNode.layout && currNode.layout.on) {
            ...
            if ("circle" == d) {
                ...
                if (currNode.selectedLocation) {
                    //纪录节点最后一次移动完成时的坐标
                    JTopo.lastMovedX = currNode.selectedLocation.x + event.dx - currNode.x;
                    JTopo.lastMovedY = currNode.selectedLocation.y + event.dy - currNode.y;
                }
                //子节点同步移动
                nodeChilds.forEach(function (n) {
                        //对于有多个节点指向的情况，为避免重复位移，每次移动幅度按比例缩小
                        n.setLocation(n.x + JTopo.lastMovedX / (n.inLinks.length > 0 ? n.inLinks.length : 1), n.y + JTopo.lastMovedY / (n.inLinks.length > 0 ? n.inLinks.length : 1));
                    }
                );
            } else {
                if ("grid" != d)
    ...
    function getNodeChilds(scene, rootNode) {
        for (var d = [], e = 0; e < scene.length; e++)
            scene[e] instanceof JTopo.Link && scene[e].nodeA === rootNode && d.push(scene[e].nodeZ);
        return d
    }
...
/************** JTopo.BarChartNode, JTopo.PieChartNode ***************/
!function (JTopo) {
    function b() {
    ...
    JTopo.BarChartNode = c;
    JTopo.PieChartNode = b;
}(JTopo);
...
/************** JTopo.Stage().find, JTopo.Scene().find ***************/
!function (JTopo) {
    function b(a, b) {
        var c = [];
        if (0 == a.length)
    ...
    function d(d) {
        var e = [],
            f = [];
        this instanceof JTopo.Stage ? (e = this.childs,
            f = f.concat(e)) : this instanceof JTopo.Scene ? e = [this] : f = this,
