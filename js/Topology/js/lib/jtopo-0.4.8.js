    ...
    var JTopo = {
                ...
                d.forEach(function (a) {
                    ...
                    if ("link" == d) {
                        ...
                        if (a["fromNode"] && a["toNode"]) {
                            nodes.forEach(function (nodeEle) {
                                if (nodeEle.elementType == "node") {
                                    if (nodeEle.id == a["fromNode"])
                                        nodeA = nodeEle;
                                    if (nodeEle.id == a["toNode"])
                                        nodeZ = nodeEle;
                                }
                            });
                        }
                        ...
                        if (nodeA && nodeZ) {
                            ...
                            else if (a["lineType"] == "pathLine") {
                                c = new JTopo.PathLink(nodeA, nodeZ);
                                c.paths = a.paths || [];
                            }

                            c.PointPathColor = JTopo.config.PointPathColor;
                            c.dashedPattern = JTopo.config.dashedPattern;
                            ...
                            if (temp && temp.length > 0) {
                                ...
                                for (var ni = 0; ni < temp.length; ni++) {
                                    b.childs.forEach(function (n) {
                                        if (n instanceof JTopo.Node && n.id == temp[ni]) {
                                            c.add(n);
                                        }
                                    });
                                }
        ...
        createStageFromJson: function (jsonObj, canvas) {
            ...
            scenes.forEach(function (a) {
                ...
                var d = a.childs;
                d.forEach(function (a) {
                    var c = null, d = a.elementType, dataType = a.dataType;
                    var p = JTopo.Element.SerializedProperties[d || "node"][dataType];
                    JTopo.util.extend(p, a);
                    if ("node" == d) {
                        c = new JTopo.Node;
                        c.alpha = JTopo.config.alpha;
                        c.strokeColor = JTopo.config.nodeStrokeColor;
...
!function (a) {
    function b(c) {
        this.initialize = function (c) {
            ...
            var b = this.text;
            window.i18n && (b = i18n.eval(this.text));
            if (null != b && "" != b) {
    ...
    g.prototype = new c, h.prototype = new c, i.prototype = new c;
    a.Node = c, a.TextNode = d, a.LinkNode = e, a.CircleNode = f, a.AnimateNode = i;
}(JTopo);

/************** JTopo.Link,JTopo.FoldLink,JTopo.FlexionalLink,JTopo.CurveLink, JTopo.PathLink ***************/
!function (a) {
    function b(a, b) {
        var c = [];
    ...
    function g(a, b, c) {
        this.initialize = function () {
            ...
            if ("horizontal" == this.direction) {
                this.nodeZ.cx > a.x ? a.x += this.nodeA.width / 2 : a.x -= this.nodeA.width / 2;
            } else {
                this.nodeZ.cy > a.y ? a.y += this.nodeA.height / 2 : a.y -= this.nodeA.height / 2;
            }
            return a;
        }, this.getEndPosition = function () {
            ...
            if ("horizontal" == this.direction) {
                ...
                a.x = this.nodeA.cx < a.x ? this.nodeZ.x : this.nodeZ.x + this.nodeZ.width;
            }
            return a;
    ...
    function j(a, b, c) {
        ...
        this.getPath = function (a) {
            ...
            d.push({x: b.x + e, y: b.y});
            if (this.paths && this.paths.length > 0) {
                this.paths.forEach(function (f) {
                    d.push({x: f[0] + e, y: f[1]});
                });
            }
            d.push({x: c.x + e, y: c.y});
            return d;
        }
    }
